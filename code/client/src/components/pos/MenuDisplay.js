...
const MenuDisplay = ({ items, onAddToOrder, showSections = false }) => {
    if (items.length === 0) {
        return (
    ...
    // Group items by category if showSections is true
    const groupedItems = showSections ? items.reduce((acc, item) => {
        const category = item.category || item.tags?.[0] || 'Other';
        if (!acc[category]) {
            acc[category] = [];
        }
    ...
    if (showSections && groupedItems) {
        return (
            <div className="p-2 h-full overflow-y-auto">
                {Object.entries(groupedItems).map(([category, categoryItems]) => (
                    <div key={category} className="mb-6">
                        <h3 className="text-lg font-bold text-gray-900 mb-3 uppercase">{category}</h3>
