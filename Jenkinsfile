            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'AWS_DEV_SSH_KEY', keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER')]) {
                    bat '''
                        ...
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "mkdir -p /home/ec2-user/pos_system/build"
                        echo "Copying build files..."
                        scp -v -i "%KEY%" -o StrictHostKeyChecking=no -r pos_system-main\\client\\build\\* %SSH_USER%@34.229.86.239:/home/ec2-user/pos_system/build/
                        echo "Verifying files were copied..."
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "cd /home/ec2-user/pos_system && ls -la build/"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "pm2 stop pos-system || true"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "cd /home/ec2-user/pos_system && pm2 serve build 3000 --name pos-system --spa"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "pm2 save"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "pm2 startup -u ec2-user --hp /home/ec2-user"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "pm2 list"
                        ssh -i "%KEY%" -o StrictHostKeyChecking=no -T %SSH_USER%@34.229.86.239 "timeout 10 pm2 logs pos-system --lines 20 || echo 'Logs command completed'"
                        del /f /q "%KEY%" >nul 2>&1
                    '''
                }
            }
